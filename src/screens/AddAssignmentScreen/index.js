...
import {sendNotification} from 'idsStore/data/notifi/operations';
...
class AddAssignmentScreen extends Component {
  ...
  toggleSubjectModal = () => {
    this.setState({showSubjectModal: !this.state.showSubjectModal});
  };
  ...
  selectSubject = async item => {
    this.setState({
      selectedSubjectName: item.label,
      showSubjectModal: false,
      selectedSubjectId: item.value,
    });
  };
  sendNotification(message, image, roleId) {
    // Sending notification
    const notificationMessage = message;
    const notificationImage = image;
    const notificationRoleId = roleId;

    // Code to send notification using props or any other method
  ...
  //     if (itemIndex === 0) {
  //       selectedClassId = this.classSectionData[0].class_id;
  //       selectedSectionId = this.classSectionData[0].section_id;
  //     } else {
  //       const selectedClassObj = this.classSectionData[itemIndex];
  //       selectedClassId = selectedClassObj.class_id;
  //       selectedSectionId = selectedClassObj.section_id;
  //     }
  ...
  handleFilePick = async () => {
    try {
      ...
      const fileSize = response.size / (1024 * 1024); // Get file size in MB
      if (fileSize > 2) {
        Alert.alert(
          'File too large',
          ...
          <>
            ...
            <ScrollView
              ...
              {this.state.selectedClassId && (
                <TouchableOpacity
                  style={styles.picker}
                  onPress={this.toggleSubjectModal}>
                  ...
                  <View style={styles.modalContent}>
                    <FlatList
                      ...
                      renderItem={({item}) => (
                        <TouchableOpacity
                          style={styles.modalItem}
                          onPress={() => this.selectSubject(item)}>
                          <Text style={styles.modalItemText}>{item.label}</Text>
                        </TouchableOpacity>
                      )}
                      keyExtractor={item => item.key.toString()}
